...
export class AnalysisService implements OnModuleInit {
  ...
  async startConversation(
    ...
    const questionHeader = ANALYSIS_MESSAGES.QUESTION_HEADER.replace('{current}', '1').replace(
      '{total}',
  ...
  async completeAnalysis(
    ...
    if (this.geminiService.isConfigured()) {
      try {
        ...
        this.logger.error(
          `Failed to generate analysis for conversation ${conversationId}: ${error.message}`
        );
  ...
  private getTemplateResponse(conversation: ConversationState): string {
    ...
    if (questionIndex < questionTemplates.length) {
      ...
      const questionHeader = ANALYSIS_MESSAGES.QUESTION_HEADER.replace(
        '{current}',
  ...
  private buildNextQuestionPrompt(
    ...
    if (questionIndex >= questionTemplates.length) {
      ...
      const completeInstruction = ANALYSIS_MESSAGES.COMPLETE_INSTRUCTION.replace(
        '{studentName}',
        studentName
      );
      return `${completeInfo} \n${completeInstruction}`;
...
CRITICAL: Ask ONLY this ONE question. STOP after this question. Do NOT continue with more questions.`;
  ...
  private validateAnalysisCompleteness(response: string): {
    ...
    try {
      ...
      if (!analysis.strengths.academic || analysis.strengths.academic.length === 0) {
        return {
          isComplete: false,
      ...
      if (!analysis.strengths.behavioral || analysis.strengths.behavioral.length === 0) {
        return {
          isComplete: false,
      ...
      if (!analysis.barriers.academic || analysis.barriers.academic.length === 0) {
        return {
          isComplete: false,
      ...
      if (!analysis.barriers.behavioral || analysis.barriers.behavioral.length === 0) {
        return {
          isComplete: false,
      ...
      // Validate goals
      if (!analysis.goals.trackingMetrics || analysis.goals.trackingMetrics.length === 0) {
        return {
          isComplete: false,
      ...
      if (
        !analysis.summary ||
        analysis.summary.trim().length < ANALYSIS_CONFIG.MIN_SUMMARY_LENGTH
      ) {
        return {
          isComplete: false,
  ...
  private generateFallbackStructuredAnalysis(conversation: ConversationState): StructuredAnalysis {
    ...
    const summary = ANALYSIS_MESSAGES.FALLBACK_SUMMARY.replace(
      '{count}',
